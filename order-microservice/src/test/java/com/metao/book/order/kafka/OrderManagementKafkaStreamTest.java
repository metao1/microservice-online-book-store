...
public class OrderManagementKafkaStreamTest {
    ...
    void shouldAggregateReservation() {
        ...
        final String orderInputTopicName = "order";
        final String productInputTopicName = "product";
        final String outputTopicName = "reservation";
        ...
        final KStream<String, OrderEvent> orderStream =
            sb.stream(orderInputTopicName, Consumed.with(Serdes.String(), orderSerds));
        final KStream<String, ProductEvent> productStream =
            sb.stream(productInputTopicName, Consumed.with(Serdes.String(), productSerds));
        ...
        try (final TopologyTestDriver testDriver = new TopologyTestDriver(sb.build(), streamProps)) {
            ...
            var productInputTopic = testDriver.createInputTopic(productInputTopicName,
                Serdes.String().serializer(),
                productSerds.serializer());
            var orderInputTopic = testDriver.createInputTopic(orderInputTopicName,
                Serdes.String().serializer(),
