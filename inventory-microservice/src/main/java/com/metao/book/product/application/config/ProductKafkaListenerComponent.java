...
public class ProductKafkaListenerComponent {
    ...
    @KafkaListener(id = "${kafka.topic.product.id}", topics = "${kafka.topic.product.name}", groupId = "${kafka.topic.product.group-id}", containerFactory = "productCreatedEventKafkaListenerContainerFactory")
