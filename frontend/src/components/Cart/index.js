...
class CartProducts extends Component {
    ...
    render() {
        const totalCost = this.state.products.length
            ? this.state.products.reduce(this.costReducer, 0) : 0;
        return (
            <div className="cart-container">
                <div className="container">
                    <h5>{this.state.isCompleted ? "Thank you!"
                        : "Items in cart"} {Boolean(this.props.cart.total) &&
                        <span
                    ...
                    {!this.props.cart.total &&
                        <h6>{this.state.isCompleted ?
                            <span>Your order <b>#kmp-{this.state.result.orderNumber}</b> is received.</span>
                            : "Cart is empty"}</h6>
                    }
                    {Boolean(this.state.isCompleted) && this.state.result &&
                        <div
                            className="order-details">{this.state.result.orderDetails}</div>
                    }
                    {Boolean(this.props.cart.total) &&
                        <div className="total">
                            ...
                            <div className="pricing">
                                {this.props.currency}<h6> {totalCost.toFixed(
                                2)} * 0.19 + {totalCost.toFixed(2)}</h6>
                                {this.props.currency}<h6> {totalCost.toFixed(
                                2)} * 0.19</h6>
                            </div>
                            <div className="actions">
                                ...
                                }} size="medium" disabled={!Boolean(
                                    this.props.cart.total)}>Checkout</Button>
                            </div>
                        </div>
                    }
                </div>
