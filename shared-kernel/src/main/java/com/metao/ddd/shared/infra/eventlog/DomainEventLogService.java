...
public class DomainEventLogService {
    ...
    public Optional<DomainEventLog> retrieveLog(@NonNull DomainEventLogId logId) {
        ...
        var events = storedDomainEventRepository.findEventsBetween(logId.low(), logId.high())
    ...
    /**
     * Returns the current log. The current log can never be completely full since
     * it becomes an archived
     * log immediately when the {@value #LOG_SIZE}:th event is
     * {@link #append(DomainEvent) appended}.
     *
